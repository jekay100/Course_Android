...
public class GestureRecognize {
	 ...
	 * @param curContext
	 *            当前的activity上下文
	 * @param myGestureLibrary
	 *            需要对比的手势库
	 * @param gesture
	 *            当前监听到的手势
	 */
	@SuppressLint("NewApi")
	static void recognize(Context curContext, GestureLibrary myGestureLibrary,
			Gesture gesture) {
		...
		if (predictions.size() > 0) {
			...
			if (prediction.score > 3.0) {
				...
				if (prediction.name.equals("next")) {
					switch (index) {
					case 1:
						curContext.startActivity(new Intent(curContext,
								GestureShow2Activity.class));
						++index;
						break;
					case 2:
						curContext.startActivity(new Intent(curContext,
								GestureShow3Activity.class));
						++index;
						break;
					case 3:
						curContext.startActivity(new Intent(curContext,
								GestureShow4Activity.class));
						++index;
						break;
					case 4:
						curContext.startActivity(new Intent(curContext,
								GestureShow1Activity.class));
						index = 1;
						break;
