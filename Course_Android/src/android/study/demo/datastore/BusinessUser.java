...
public class BusinessUser {
	...
	public Boolean DeleteUser(String p_Condition) {
		return m_SQLiteDatabase.delete(GetTableNameAndPK()[0], " 1=1 "
				+ p_Condition, null) >= 0;
	}
	...
	public Boolean UpdateUser(String p_Condition, ModelUser p_Info) {
		ContentValues _ContentValues = CreatParms(p_Info);
		return m_SQLiteDatabase.update("User", _ContentValues, p_Condition,
				null) > 0;
	}
	...
	protected ModelUser FindModel(Cursor p_Cursor) {
		...
		_ModelUser
				.SetUserID(p_Cursor.getInt(p_Cursor.getColumnIndex("UserID")));
		_ModelUser.SetUserName(p_Cursor.getString(p_Cursor
				.getColumnIndex("UserName")));
		SimpleDateFormat _DateFormat = new SimpleDateFormat(
				"yyyy/MM/dd HH:mm:ss:SSS");
		Date _CreateDate;
		try {
			_CreateDate = _DateFormat.parse(p_Cursor.getString(p_Cursor
		...
		_ModelUser
				.SetState((p_Cursor.getInt(p_Cursor.getColumnIndex("State"))));
	...
	public ContentValues CreatParms(ModelUser p_Info) {
		...
		SimpleDateFormat _DateFormat = new SimpleDateFormat(
				"yyyy/MM/dd HH:mm:ss:SSS");
		_ContentValues.put("CreateDate",
				_DateFormat.format(p_Info.GetCreateDate()));
