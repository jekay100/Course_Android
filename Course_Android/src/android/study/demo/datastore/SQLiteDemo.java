...
public class SQLiteDemo extends Activity {
	...
	private class OnDeleteClickListener implements
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			// 获取点击行的User实体类
			ModelUser _ModelUser = (ModelUser) _AdapterUser
					.getItem(m_ListViewPosition);
			BusinessUser _BusinessUser = new BusinessUser(SQLiteDemo.this);
			// 调用删除方法删除
			Boolean _Result = _BusinessUser.DeleteUserByUserID(_ModelUser
					.GetUserID());
			if (_Result == true) {
				// 移除删除要删除的行
	...
	protected AlertDialog ShowAlertDialog(String p_Title, String p_Message,
		...
		return new AlertDialog.Builder(this).setTitle(p_Title)
				.setMessage(p_Message).setPositiveButton("确定", p_ClickListener)
	...
	public void onCreateContextMenu(ContextMenu p_ContextMenu, View p_View,
		...
		ModelUser _ModelUser = (ModelUser) _ListAdapter
	...
	public boolean onContextItemSelected(MenuItem p_Item) {
		AdapterContextMenuInfo _AdapterContextMenuInfo = (AdapterContextMenuInfo) p_Item
				.getMenuInfo();
		ListAdapter _ListAdapter = lvUser.getAdapter();
		ModelUser _ModelUser = (ModelUser) _ListAdapter
				.getItem(_AdapterContextMenuInfo.position);
	...
	protected void ShowUserAddOrEditDialog(ModelUser p_ModelUser) {
		...
		_Builder.setNeutralButton("保存", new OnAddOrEditUser(p_ModelUser,
				_etUserName, true));
	...
	// 关闭对话框
	protected void SetAlertDialogIsClose(DialogInterface p_Dialog,
			Boolean p_IsClose) {
		try {
			Field _Field = p_Dialog.getClass().getSuperclass()
	...
	private class OnAddOrEditUser implements DialogInterface.OnClickListener {
		...
		public OnAddOrEditUser(ModelUser p_ModelUser, EditText p_txtUserName,
				Boolean p_IsSaveButton) {
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			// 判断是否已经存在该用户名
			boolean _CheckResult = !_BusinessUser.IsExistUserByUserName(
					_UserName, m_ModelUser.GetUserID());
			if (!_CheckResult) {
			...
			m_ModelUser.SetUserName(String.valueOf(m_txtUserName.getText()
					.toString().trim()));
